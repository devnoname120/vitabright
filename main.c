...
static tai_hook_ref_t power_set_max_brightness_ref = -1;

int hook_kscePowerSetDisplayMaxBrightness(int limit) {
    return TAI_CONTINUE(int, power_set_max_brightness_hook, 0x10000);
}
...
int module_start(SceSize argc, const void *args) {
  ...
  power_set_max_brightness_hook = taiHookFunctionExportForKernel(KERNEL_PID, &power_set_max_brightness_ref, "ScePower", 0x1590166F, 0x77027B6B, hook_kscePowerSetDisplayMaxBrightness);
